...
def kpi_dashboard():
    ...
    # 4. LẤY DANH SÁCH NHÂN SỰ (Lọc theo Division và loại bỏ 9. DU HOC)
    user_division = session.get('division', '5000') # Mặc định STDD
    query_users = """
        SELECT USERCODE, SHORTNAME, [BO PHAN] as Department 
        FROM [dbo].[GD - NGUOI DUNG] 
        WHERE [BO PHAN] IS NOT NULL AND REPLACE([BO PHAN], ' ', '') <> '9.DUHOC' AND Division = ?
        ORDER BY [BO PHAN], SHORTNAME
    """
    all_users = db.get_data(query_users, (user_division,))
        
    # 5. Lấy AvatarUrl từ DB (Fix đúng tên cột AvatarUrl)
    ...
    try:
        ...
        current_app.logger.error(f"Lỗi lấy avatar KPI: {e}")

    # 6. Lấy kết quả KPI
    kpi_results = kpi_service.get_kpi_results_for_view(target_user, selected_year, selected_month)
