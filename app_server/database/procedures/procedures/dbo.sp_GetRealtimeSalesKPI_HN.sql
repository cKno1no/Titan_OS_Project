...
create PROCEDURE [dbo].[sp_GetRealtimeSalesKPI_HN]
...
BEGIN
    ...
    FROM [OMEGA_TEST].[dbo].[OT2001] AS T1
    INNER JOIN [OMEGA_TEST].[dbo].[OT2002] AS T2 ON T1.SOrderID = T2.SOrderID
    LEFT JOIN [OMEGA_TEST].[dbo].[IT1202] AS T3 ON T1.ObjectID = T3.ObjectID
    ...
    FROM [OMEGA_TEST].[dbo].[OT2001] AS T1 
    LEFT JOIN [OMEGA_TEST].[dbo].[IT1202] AS T2 ON T1.ObjectID = T2.ObjectID 
    ...
    -- =========================================================================
    IF EXISTS (
        SELECT 1 FROM [OMEGA_TEST].[dbo].[OT2101] AS T1 
    ...
    BEGIN
        SELECT TOP 10
            ...
            T2.ShortObjectName AS ClientName 
        FROM [OMEGA_TEST].[dbo].[OT2101] AS T1 
        LEFT JOIN [OMEGA_TEST].[dbo].[IT1202] AS T2 ON T1.ObjectID = T2.ObjectID 
        WHERE T1.QuotationDate BETWEEN @StartOfMonth AND @EndOfMonth AND T1.OrderStatus = 1 
            AND (@SalesmanID IS NULL OR RTRIM(T1.SalesManID) = RTRIM(@SalesmanID))
        ORDER BY T1.SaleAmount DESC;
    ...
    ELSE
    BEGIN
        SELECT TOP 0 CAST(NULL AS NVARCHAR(10)) AS VoucherNo, CAST(NULL AS DATE) AS QuoteDate, 0.0 AS QuoteAmount, CAST(NULL AS NVARCHAR(100)) AS ClientName FROM [OMEGA_TEST].dbo.GT9000 WHERE 1=0; 
    END
    ...
    IF EXISTS (
        SELECT 1 FROM [OMEGA_TEST].[dbo].[OT2002] AS T1 
        INNER JOIN [OMEGA_TEST].[dbo].[OT2001] AS T2 ON T1.SOrderID = T2.SOrderID
    ...
    BEGIN
        ...
        FROM [OMEGA_TEST].[dbo].[OT2002] AS T1 
        INNER JOIN [OMEGA_TEST].[dbo].[OT2001] AS T2 ON T1.SOrderID = T2.SOrderID
        LEFT JOIN [OMEGA_TEST].[dbo].[IT1202] AS T3 ON T2.ObjectID = T3.ObjectID
        LEFT JOIN [OMEGA_TEST].[dbo].[IT1302] AS T4 ON T1.InventoryID = T4.InventoryID
    ...
    BEGIN
        ...
        FROM [OMEGA_TEST].dbo.GT9000 WHERE 1 = 0; 
    END
