...
create PROCEDURE [dbo].[sp_GetSalesLookup_Common_HN]
...
BEGIN
    ...
    LEFT JOIN [OMEGA_TEST].[dbo].[CRM_TON KHO BACK ORDER] AS T2
    ...
    WITH RankedQuotes AS (
        ...
        INNER JOIN [OMEGA_TEST].[dbo].[OT2101] AS T2 ON T1.QuotationID = T2.QuotationID
        INNER JOIN #InvIDs T_Filter ON T1.InventoryID = T_Filter.InventoryID
        WHERE 
