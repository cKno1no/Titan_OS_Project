...
CREATE PROCEDURE [dbo].[sp_GetSalesLookup_Block1_HN]
...
BEGIN
    ...
    INTO #MatchingItems
    FROM [OMEGA_TEST].[dbo].[IT1302] AS T1
    WHERE EXISTS (
    ...
    LEFT JOIN (
        ...
        FROM [OMEGA_TEST].[dbo].[CRM_TON KHO BACK ORDER]
    ...
    FROM (
        ...
        FROM [OMEGA_TEST].[dbo].[CRM_TV_THONG TIN DHB_FULL] AS T1
    ...
    FROM (
        ...
        FROM [OMEGA_TEST].[dbo].[OT2102] AS T1
        JOIN [OMEGA_TEST].[dbo].[OT2101] AS T2 ON T1.QuotationID = T2.QuotationID
        INNER JOIN #MatchingItems T_Filter ON T1.InventoryID = T_Filter.InventoryID
        WHERE 
    ...
    FROM [OMEGA_TEST].[dbo].[CRM_TV_THONG TIN DHB_FULL]
    WHERE 
    ...
    FROM [OMEGA_TEST].[dbo].[CRM_TV_THONG TIN DHB_FULL 2]
    WHERE 
        EXISTS ( 
            SELECT 1 FROM #MatchingItems T_Filter 
