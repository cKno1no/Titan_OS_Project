CREATE PROCEDURE [dbo].[sp_GetQuotesForApproval_Optimized_HN]
...
BEGIN
    ...
    -- 1. Lấy danh sách VoucherType mà User được quyền duyệt (nếu ko phải Admin)
    DECLARE @UserPermissions TABLE (VoucherTypeID NVARCHAR(20));
    
    IF @IsAdmin = 0
    BEGIN
        INSERT INTO @UserPermissions (VoucherTypeID)
        SELECT VoucherTypeID FROM [OT0006] WHERE LOWER(Approver) = LOWER(@UserCode);
    END
    ...
    LEFT JOIN [OMEGA_TEST].[dbo].[OT2102] AS T4 ON T1.QuotationID = T4.QuotationID 
    LEFT JOIN [OMEGA_TEST].[dbo].[IT1302] AS T5 ON T4.InventoryID = T5.InventoryID        
    ...
    WHERE T1.OrderStatus = 0 
      AND T1.QuotationDate BETWEEN @DateFrom AND @DateTo
      -- Logic phân quyền: Nếu Admin lấy hết, không phải Admin thì Join bảng quyền
      AND (@IsAdmin = 1 OR T1.VoucherTypeID IN (SELECT VoucherTypeID FROM @UserPermissions))
