...
# 6. Global Before Request (Chạy trước mỗi request)
    ...
    def auto_audit_logger(response):
        ...
        # [CẬP NHẬT]: Thêm các route spam vào danh sách loại trừ
        exclude_paths = [
            '/static', 
            '/api/pet/', 
            '/api/training/progress',  # Chặn spam lưu trang đọc (3s/lần)
            '/api/training/status',    # Chặn các API check status định kỳ
            '/api/mailbox'             # Chặn log khi load hòm thư (nếu dùng POST)
        ]
