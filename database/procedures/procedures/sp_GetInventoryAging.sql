...
CREATE PROCEDURE sp_GetInventoryAging
...
BEGIN
    ...
    -- Khai báo Bảng ERP
    DECLARE @IT1301 NVARCHAR(100) = N'[OMEGA_STDD].[dbo].[IT1301]';
    DECLARE @IT1302 NVARCHAR(100) = N'[OMEGA_STDD].[dbo].[IT1302]';
    DECLARE @WT2008 NVARCHAR(100) = N'[OMEGA_STDD].[dbo].[WT2008]'; 
    ...
    WITH LastReceiptDate AS (
        ...
        FROM [OMEGA_STDD].[dbo].[WT2006] AS T1 
        INNER JOIN [OMEGA_STDD].[dbo].[WT2007] AS T2 ON T1.VoucherID = T2.VoucherID
        WHERE T1.VoucherTypeID = ''PN''
    ...
    WHERE 
        T_CUR.WareHouseID IN (''Q4'', ''TG15'', ''PM'')
